...
# Criação do banco e categorias padrão apenas se não existirem
def initialize_database():
    with app.app_context():
        # Verifica se o banco já foi criado
        if not os.path.exists('instance') or not db.engine.dialect.has_table(db.engine, 'ganho'):
            db.create_all()
            
            # Adiciona categorias padrão apenas se a tabela estiver vazia
